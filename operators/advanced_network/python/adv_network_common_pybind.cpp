...
PYBIND11_MODULE(_advanced_network_common, m) {
    ...
    m.def("adv_net_create_burst_params", &adv_net_create_burst_params, py::return_value_policy::reference, "Create a shared pointer burst params structure");
    ...
    m.def("adv_net_tx_burst_available", py::overload_cast<AdvNetBurstParams *>(&adv_net_tx_burst_available), "Return true if a TX burst is available for use");
    m.def("adv_net_tx_burst_available", py::overload_cast<std::shared_ptr<AdvNetBurstParams>>(&adv_net_tx_burst_available), "Return true if a TX burst is available for use");
    ...
    // m.def("adv_net_set_cpu_udp_hdr_payload", 
    //     [](AdvNetBurstParams *burst, int idx, long int data, int len) { return adv_net_set_cpu_udp_hdr_payload(burst, idx, reinterpret_cast<void*>(data), len); }, "Set UDP header parameters and copy payload");
    // m.def("adv_net_set_cpu_udp_hdr_payload", 
    //     [](std::shared_ptr<AdvNetBurstParams> burst, int idx, long int data, int len) { return adv_net_set_cpu_udp_hdr_payload(burst, idx, reinterpret_cast<void*>(data), len); }, "Set UDP header parameters and copy payload");
    ...
    m.def("adv_net_get_q_id", py::overload_cast<AdvNetBurstParams *>(&adv_net_get_q_id), "Get queue ID of a burst");
    m.def("adv_net_get_q_id", py::overload_cast<std::shared_ptr<AdvNetBurstParams> >(&adv_net_get_q_id), "Get queue ID of a burst");
    ...
    py::class_<AdvNetBurstHdr>(m, "AdvNetBurstHdr").def(py::init<>());
    py::class_<AdvNetBurstParams>(m, "AdvNetBurstParams").def(py::init<>());
    //py::class_<AdvNetBurstParams, std::shared_ptr<AdvNetBurstParams>>(m, "AdvNetBurstParams").def(py::init<>());
